...
<head>
    ...
    <script>
        ...
        function addStopwatch(name = "", description = "", time = 0, running = false) {
            ...
            div.innerHTML = `
				<button id= "start-stop-btn" onclick="toggleStopwatch(this)">${running ? "Stop" : "Start"}</button>
				<button onclick="removeStopwatch(this)">Remove</button><br>
                <span class="time">${formatTime(time)}</span><br>
                <input type="text" value="${name}" placeholder="Stopwatch Name"><br>
                <textarea placeholder="Description">${description}</textarea><br><br><br>
            `;
        ...
        function exportToTxt() {
            ...
            document.querySelectorAll(".stopwatch").forEach(div => {
                stopwatches.push({
                    name: div.querySelector("input").value,
                    description: div.querySelector("textarea").value,
                    time: div.querySelector(".time").textContent
                });
            });
...
<body>
    ...
    <button onclick="clearAllStopwatches()">Clear All</button>
